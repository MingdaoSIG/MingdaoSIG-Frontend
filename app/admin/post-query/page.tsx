...
export default function Page() {
  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      const users = await Promise.all(data.map((d: any) => sigAPI.getUserData(d.user)));
      const userMap = users.reduce((acc: any, user: any) => {
        if (user) acc[user._id] = user.name;
        return acc;
      }, {});
      setResults(data.map((d: any) => ({ ...d, userName: userMap[d.user] || "未知" })));
    } catch (e) {
      setResults([]);
    } finally {
      setLoading(false);
        ...
        <div className="max-w-md mx-auto">
          ...
          <div className="bg-white rounded-xl shadow-md p-4 mb-4 border border-gray-100">
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">開始日期</label>
                <input 
                  type="date" 
                  value={startDate} 
              ...
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">結束日期</label>
                <input 
                  type="date" 
                  value={endDate} 
          ...
          {/* Results Card */}
          <div className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden" style={{ height: 'calc(100vh - 320px)', minHeight: '300px' }}>
            <div className="overflow-auto h-full">
              <table className="w-full text-sm">
                <thead className="bg-gradient-to-r from-cyan-500 to-blue-500 text-white sticky top-0">
                  <tr>
                    <th className="py-3 px-2 w-[20%] text-center font-semibold text-xs">ID</th>
                    <th className="py-3 px-2 w-[45%] text-left font-semibold text-xs">標題</th>
                    <th className="py-3 px-2 w-[15%] text-center font-semibold text-xs">作者</th>
                    <th className="py-3 px-2 w-[20%] text-center font-semibold text-xs">時間</th>
                  </tr>
                </thead>
                  ...
                  {results.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="text-center py-12">
                        ...
                        <p className="text-xs text-gray-400 mt-1">請選擇日期範圍後查詢</p>
                      </td>
                    </tr>
                  ) : (
                    results.map((r, i) => (
                      <tr key={r._id} className={i % 2 === 0 ? "bg-gray-50/50" : "bg-white"}>
                        <td className="py-3 px-2 text-center">
                          <span className="text-xs bg-cyan-100 text-cyan-700 px-1.5 py-0.5 rounded">{r._id?.slice(-6) || "N/A"}</span>
                        </td>
                        <td className="py-3 px-2">
                          <div className="truncate max-w-[150px]" title={r.title}>{r.title}</div>
                        </td>
                        <td className="py-3 px-2 text-center text-gray-600">{r.userName}</td>
                        <td className="py-3 px-2 text-center text-gray-500 text-xs">{formatDateTime(r.createdAt)}</td>
                      </tr>
                    ))
                  )}
                </tbody>
          ...
          <div className="bg-gray-50 px-4 py-3 text-center text-sm rounded-xl border border-gray-200 mt-3">
            找到 <strong className="text-cyan-600">{results.length}</strong> 筆資料
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="w-full h-[calc(100%-2rem)] pt-4 px-4 overflow-hidden flex flex-col">
      <h1 className="text-2xl font-bold text-gray-800 text-center mb-2 flex-shrink-0">區間資料查詢</h1>
      <div className="flex-shrink-0 flex justify-center gap-4 mb-2">
        <div>
          <label className="text-sm font-medium text-gray-700">開始日期</label>
          <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="block px-3 py-1.5 border border-gray-300 rounded-lg focus:outline-none" />
        </div>
        <div>
          <label className="text-sm font-medium text-gray-700">結束日期</label>
          <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="block px-3 py-1.5 border border-gray-300 rounded-lg focus:outline-none" />
        </div>
      </div>
      <div className="flex-shrink-0 text-center mb-2">
        <button onClick={fetchPosts} disabled={loading} className="px-6 py-2 bg-blue-500 hover:bg-blue-600 disabled:bg-blue-300 text-white rounded-lg cursor-pointer disabled:cursor-not-allowed transition-colors">
          {loading ? "查詢中..." : "查詢"}
        </button>
        {results.length > 0 && <button onClick={downloadXLSX} className="ml-2 px-6 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg cursor-pointer transition-colors">下載 XLSX</button>}
      </div>
        ...
        <div className="overflow-auto h-full">
          <table className="w-full">
            <thead className="bg-gradient-to-r from-blue-500 to-blue-600 text-white sticky top-0">
              <tr>
                <th className="py-3 px-2 w-[20%] text-center font-semibold">ID</th>
                <th className="py-3 px-2 w-[58%] text-center font-semibold">標題</th>
                <th className="py-3 px-2 w-[10%] text-center font-semibold">作者</th>
                <th className="py-3 px-2 w-[12%] text-center font-semibold">建立時間</th>
              </tr>
            </thead>
              ...
              {results.length === 0 ? (
                ...
                results.map((r, i) => (
                  <tr key={r._id} className={i % 2 === 0 ? "bg-gray-50" : "bg-white"}>
                    <td className="py-3 px-2 text-center">
                      <span className="bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full text-xs">{r._id || "N/A"}</span>
                    </td>
